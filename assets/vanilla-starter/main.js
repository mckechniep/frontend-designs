...
const CYBER_PROFILE_ID = "cyberpunk-neon";
...
const RETRO_NAV_COMMANDS = {
  ...
  "./components.html": "whoami --verbose",
};
...
function isCyberpunkActive() {
  return (
    normalizeProfileId(document.documentElement.dataset.profile || getFixedProfileId() || "") ===
    CYBER_PROFILE_ID
  );
}
...
function ensureNoireCursorDot(active) {
  ...
  if (!noireState.cursorDot) {
    ...
    document.addEventListener(
      "pointermove",
      (event) => {
        const target = event.target instanceof Element ? event.target.closest("[data-noire-cursor-scope]") : null;
        noireState.cursorInScope = Boolean(target);
    ...
    document.addEventListener(
      "pointerleave",
      () => {
        noireState.cursorInScope = false;
      },
      { passive: true }
    );
...
function ensureNoireRedaction(active) {
  ...
  const redacted = qsa(".noire-redacted-line", lead);

  if (!active) {
    redacted.forEach((line) => line.classList.remove("revealed"));
  ...
  redacted.forEach((line) => {
    if (line.dataset.noireBound === "true") return;
    line.addEventListener("mouseenter", () => line.classList.add("revealed"), { once: true });
    line.dataset.noireBound = "true";
  });
  ...
  noireState.redactionTimerId = window.setTimeout(() => {
    redacted.forEach((line) => line.classList.add("revealed"));
  }, 2400);
}
...
function runNoireRevealSequence() {
  ...
  const targets = [
    ...
    ...qsa(".section .metric"),
    ...qsa(".contact-card-wrapper .card"),
    ...qsa(".site-footer .noire-footer-column, .site-footer .noire-footer-meta"),
  ];
  if (targets.length === 0) return;
...
function initCursorSpotlightTracking() {
  ...
  const ensureCyberpunkSpotlightLayer = () => {
    let layer = qs(".cyber-cursor-spotlight");
    if (!layer) {
      layer = document.createElement("div");
      layer.className = "cyber-cursor-spotlight";
      layer.hidden = true;
  ...
  const syncCyberpunkSpotlightLayer = () => {
    const existingLayer = qs(".cyber-cursor-spotlight");
    const active =
      !isCoarsePointer() &&
      isCyberpunkActive() &&
      root.classList.contains("effects-effect-cursor-spotlight");

    if (!active) {
      if (existingLayer) existingLayer.hidden = true;
      return existingLayer;
    }

    const layer = existingLayer || ensureCyberpunkSpotlightLayer();
    layer.hidden = false;
    return layer;
  };
  const resetSpotlight = () => {
    ...
    syncCyberpunkSpotlightLayer();
  };

  const updateSpotlight = (event) => {
    if (!root.classList.contains("effects-effect-cursor-spotlight")) {
      syncCyberpunkSpotlightLayer();
      return;
    }
  ...
  window.addEventListener("blur", resetSpotlight, { passive: true });
  resetSpotlight();
}
